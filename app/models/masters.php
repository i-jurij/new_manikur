...
class Masters extends Home
{
	...
	public function change($path)
	{
		...
		if (!empty($_POST['master'])) {
            ...
            if (empty($this->data['master'])) {
                $this->data['master'] = '1';
            }
            if (empty($this->data['uv_master'])) {
                $this->data['uv_master'] = '1';
            }
		}
		return $this->data;
	}
    ...
    public function change_photo($path)
	{
		...
		if (!empty($_POST['change_photo'])) {
			...
			$data = $this->get_data_masters();
            if (empty($data)) {
				$this->data['choose_master'] = "Список мастеров пуст.";
			} else {
				$this->data['choose_master'] = $data;
			}
		}
		return $this->data;
	}
